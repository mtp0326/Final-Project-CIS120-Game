 ...
 */
public class MandM extends GameObj {
    ...
    private static int initPosY = (int) (Math.random() * (GameCourt.COURT_HEIGHT - (SIZE) * 2)
            + SIZE);
    private static int initVelX = (int) (Math.random() * 9 + 13 + GameCourt.wave * 2) * -1;
    private static int initVelY = 0;
    private static int number = 2;

    public static final String IMG_ENEMY_FILE = "files/mm.png";

    private static BufferedImage img;

    public MandM(int courtWidth, int courtHeight, int x, int y, int vx, int vy) {
        ...
        try {
            if (img == null) {
                img = ImageIO.read(new File("files/mm.png"));
            }
        } catch (IOException e) {
